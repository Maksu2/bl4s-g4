...
===================================================
Inteligentne wznawianie przerwanej sesji z elastycznym `batchowaniem`.
- Cu 2 GeV wznowione od grubości 12.1 cm (do 30.0 cm)
- Reszta (Cu 3-6 GeV, Pb 2-6 GeV) obrabiana standardowo od 0.1 do 30.0 cm w krokach co 0.1.

Usage:
  source geant4_install/bin/geant4.sh
  ./compile2.sh (Raz, by przygotować binarkę z O3/march=native)
  python3 run_new2.py
"""
...
JOBS = []
# Cu @ 2 GeV zrobione do 120 -> grubość 12.0 cm, wznawiamy od 12.1 cm:
JOBS.append(("G4_Cu", "Cu", "2 GeV", 12.1, 30.0))

# Reszta Miedzi (Cu) normalnie od 0.1 do 30 cm:
for e in [3, 4, 5, 6]:
    JOBS.append(("G4_Cu", "Cu", f"{e} GeV", 0.1, 30.0))

# Ołów (Pb) normalnie od 0.1 do 30 cm:
for e in [2, 3, 4, 5, 6]:
    JOBS.append(("G4_Pb", "Pb", f"{e} GeV", 0.1, 30.0))
...
<div class="header-wrapper">
  <div>
    ...
    <p class="sub">6 PARALLEL PROCESSES × 1 THREAD | OPTIMIZED RESUME</p>
  </div>
...
<div class="footer">Auto-sync active • Dashboard RESUME Glass</div>
...
def main():
    ...
    master = f"Results_Resume_{timestamp}"
    ...
    print(f"""
\033[36m╔══════════════════════════════════════════════════════════════╗
║         ⚛  GEANT4 SIMULATION - RESUME BUILD  ⚛              ║
╚══════════════════════════════════════════════════════════════╝\033[0m
