#!/usr/bin/env python3
"""
Geant4 Lead (Pb) 2 GeV Simulation Runner + Live Web Dashboard
=====================================================
Custom version for Lead (Pb) simulations:
- Material: G4_Pb
...
<title>Geant4 Pb(2GeV) — Live Dashboard</title>
...
<div class="header-wrapper">
  <div>
    <h1>⚛ GEANT4 LEAD SIMULATION</h1>
    <p class="sub">6 PARALLEL PROCESSES × 1 THREAD | MATERIAL: Pb @ 2 GeV</p>
  </div>
...
<div class="table-wrap">
  <h2 style="font-size:14px;font-weight:600;margin-bottom:16px;color:var(--text);letter-spacing:1px">BATCH QUEUE (Pb)</h2>
  <table>
...
<div class="footer">Auto-sync active • GeantSim Pb Dashboard</div>
...
def main():
    ...
    master = f"Results_Pb2GeV_{timestamp}"
    os.makedirs(master, exist_ok=True)

    summary = os.path.join(master, "summary_pb.csv")
    ...
    print(f"""
\033[36m╔══════════════════════════════════════════════════════════════╗
║         ⚛  GEANT4 SIMULATION - LEAD (Pb) 2 GeV ⚛            ║
╚══════════════════════════════════════════════════════════════╝\033[0m
  Material:                    \033[97mLead (G4_Pb)\033[0m
...
""")
    ...
    for batch_idx, (mat_code, mat_short, energy, t_from, t_to) in enumerate(JOBS):
        ...
        def process_thickness(args):
            ...
            csv_path, hits = run_single(mac, sub, f"pb2_{batch_idx}_{idx}")
