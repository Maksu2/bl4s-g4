...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.entry_threads = self.create_input("Threads", str(cores))
        
        row1.addWidget(self.entry_electrons)
        row1.addWidget(self.entry_energy)
        row1.addWidget(self.entry_threads)
        ...
        self.combo_mat.addItems(["Lead (Pb)", "Copper (Cu)"])
        self.combo_mat.setStyleSheet("background-color: transparent; border: 1px solid #333333; color: #E0E0E0; padding: 5px;")
        
        widget_mat = QWidget()
        l_mat = QVBoxLayout(widget_mat)
        l_mat.setContentsMargins(0,0,0,0)
        l_mat.setSpacing(5)
        lbl_mat = QLabel("Target Material")
        lbl_mat.setObjectName("FieldLabel")
        l_mat.addWidget(lbl_mat)
        l_mat.addWidget(self.combo_mat)
    ...
    def add_to_queue(self):
        ...
        thr = self.entry_threads.entry.text()
        mat = "G4_Pb" if self.combo_mat.currentIndex() == 0 else "G4_Cu"
